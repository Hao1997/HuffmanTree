...
public class HuffmanTree {
 ...
 private HuffmanNode BuildTree(int[] frequencies,char[] letters) {
	...
	while (heap.size()!=1){
		Entry<HuffmanNode, HuffmanNode> node1 = heap.removeMin();
		Entry<HuffmanNode, HuffmanNode> node2 = heap.removeMin();
		
		if(node1!=null&&
		node2!=null){
		HuffmanNode combine = new HuffmanNode(0,node1.getValue().getFrequency() + node2.getValue().getFrequency(), null, node1.getValue(), node2.getValue());
		node1.getValue().setParent(combine);
		node2.getValue().setParent(combine);
		heap.insert(combine, combine);
		}
	}
	
	root = heap.removeMin().getValue();
	return root;
	...
	//return null; /***** this must be altered to return the root of the tree build by steps 6-13 ****/
...
private String encodeCharacter(int c) {
	 HuffmanNode start = leafWhereLetterIs[c];
	 ...
	 while(start.parent()!=null){
		 parent = start.parent();
		 if(parent.leftChild().equals(start)){
			 bitRepresentation = "0" + bitRepresentation;
		 }else if(parent.rightChild().equals(start)){
	 ...
	 return bitRepresentation;   /*** note this is returning a wrong output (always 0)***/
 } 
...
public int decodeCharacter(Iterator<Byte> bit) {
	...
	HuffmanNode iterator = this.root;
	 int i = 0;
	  while (!iterator.isLeaf()){
		  Byte compare = bit.next();
		
		 if((compare.equals(new Byte("1"))) ){
